...
public class SixCimTwoSpeed extends Subsystem {
	...
	private void shiftAutomatically(){
		...
		if(getDriveSpeedFPS() > Constants.drivetrainUpshiftSpeedThreshold && Math.abs(power) > Constants.drivetrainUpshiftPowerThreshold && isSpeedingUp() &&isInLowGear()){
			//Normal Upshift
			//if fast enough to need to upshift, driver is applying sufficient throttle, the robot is speeding up and it's in low gear, upshift
